...
export default function DashboardAidePage() {
  ...
  const { t } = useLang();
  const backHref = user
    ? getDashboardPathForRole(getPrimaryRole(user.roles) ?? user.roles[0])
    : "/dashboard";
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-6">
        <Link
          href={backHref}
          className="inline-flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground"
        >
      ...
      <section className="mb-10 rounded-2xl border border-border bg-gradient-to-b from-primary/5 to-transparent p-6 sm:p-8">
        <h2 className="font-display text-2xl font-bold text-foreground sm:text-3xl">
          {t("Centre d'aide")}
        </h2>
        <p className="mt-2 max-w-2xl text-muted-foreground">
          {t("Trouvez des réponses à vos questions ou contactez notre équipe.")}
        </p>
      </section>
